...
st.write("""
Приложение *"Lukoil stock price prediction"* помогает наглядно продемонстрировать студентам основы использования 
нейронных сетей на примере прогнозирования цен акций Лукойла, исходя из данных предыдущих периодов. 
\nВ приложении **3 блока**: **работа с данными**, прогноз цены **загруженной моделью** и **создание нейросети "с нуля"** самостоятельно, с последующим обучением.
\nВ *первом блоке* студенты смотрят изначальные данные по ценам акций компании Лукойл: различные показатели цен в разные промежутки времени. Далее демонстрируются  возможности для графического анализа данных и их обработка для внесения в модель.
\nВо *втором блоке* представлено описание уже обученной модели, её загрузка и прогноз данных с его графическим отображением.
\nВ *третьем блоке* создаётся полносвязанная нейронная сеть с 3мя слоями, происходит её компиляция и обучение - по количеству выбранных студентом эпох. Далее графически отображается значения loss-функции при обучении и прогноз модели.
